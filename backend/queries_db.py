...
def add_notification(user_id: int, from_user_id: int, message: str,
                     amount: float = None, description: str = None):
    with get_db() as conn:
        cursor = conn.execute(
            """INSERT INTO notifications (user_id, from_user_id, message, amount, description)
               VALUES (?, ?, ?, ?, ?)""",
            (user_id, from_user_id, message, amount, description)
        )
        return cursor.lastrowid
...
def get_notifications(user_id: int, limit: int = 50):
    with get_db() as conn:
        rows = conn.execute(
            """SELECT n.*, u.real_name as from_user_name
