...
def index():
    ...
    if current_group_id == "personal":
        ...
        # Получаем долги и участников группы
        debts = get_group_debts(group_id)
        group_members = get_group_members_for_select(group_id)
...
def add_debt_route():
    ...
    if not group_id or group_id == "personal":
        return redirect(url_for("index"))
    ...
    if debtor_id == creditor_id:
        flash("Нельзя добавить долг самому себе", "error")
        return redirect(url_for("index", group=group_id))

    add_debt(group_id, debtor_id, creditor_id, amount, description)
    return redirect(url_for("index", group=group_id))

@app.route("/delete-debt/<int:debt_id>", methods=["POST"])
